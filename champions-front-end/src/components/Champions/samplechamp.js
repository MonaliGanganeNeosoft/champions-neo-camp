...
class ChampionOfTheWeek extends Component {
  ...
  getChamionOfWeekList = () => {
    API.getChampionOfWeek(this.getChamionOfWeekCallback, {
  ...
  handleWinner = () => {
    this.editor.exportHtml((data) => {
      ...
      let date = new Date(
        new Date(this.state.voterInfo.date).getFullYear(),
        new Date(this.state.voterInfo.date).getMonth(),
        new Date(this.state.voterInfo.date).getDate()
      );
      console.log();
      API.updateChampionOfWeek(
        this.updateChampionOfWeekCallback,
        { status: true, weekDate: date, htmlContent: html },
        this.state.voterInfo._id
      );
    });
  };
  ...
  weekAndDay(date) {
    ...
    return (
      prefixes[Math.floor(weekDay.getDate() / 7)] +
      " Week Of " +
      month +
      " " +
      weekDay.getFullYear()
          ...
          >
            <DialogTitle id="alert-dialog-title">
              {"Please compose Email for champion" +
                " " +
                this.state.voterInfo.champId.firstName +
                " " +
                this.state.voterInfo.champId.lastName +
                " of the week"}
            </DialogTitle>
            <DialogContent>
