...
class AddEditChampion extends Component {
    constructor() {
        super() 
        this.state = {
            isLoading: false,
    ...
    componentDidMount() {
        ...
        let { userData, championData } = this.props;
        if (userData) {
            let { name } = userData;
            let { firstName, lastName, location, department, emailId, id} = championData;
            if (championData.emailId) {
            this.setState({
                header: "Welcome " + name,
                firstName: firstName,
                lastName: lastName,
    ...
    handleOnChange = ({ target: { name, value } }) => {
      ...
      handleSubmit = e => {
        ...
        if(this.handleValidation()){
          ...
          let {championData} = this.props;
          if (championData.emailId) {
            API.updateChampion(this.getAuthenticationCallback, {firstName: firstName, lastName: lastName, department: department, location: location, email: emailId}, updateId);
          } else {
            API.addChampion(this.getAuthenticationCallback, {firstName: firstName, lastName: lastName, department: department, location: location, email: emailId, leadEmail: this.props.userData.emailId});
          }
        }
        else {
        ...
        return (
            <form onSubmit={this.handleSubmit}>
                <div className='leadFormDiv'>
                    <div className="heading">Add Champion</div>
...
const ChampionActionConatiner = connect(
    mapStateToProps,
    mapDispatchToProps
)(AddEditChampion);

export default ChampionActionConatiner;
